---
layout: post
title: "mawk or awk? Munging large log files"
date: 2014-02-14 09:26
comments: true
categories:
- mawk
- awk
...
I've recently needed to process some pretty large log files.  The files accrue about 2gb a week.  For now, it's possible to read the files into ```R```, but very soon it won't be possible without a key-value store, or serving it off of a MySQL server [as Jim Porzack suggests.](http://brenocon.com/blog/2009/02/comparison-of-data-analysis-packages-r-matlab-scipy-excel-sas-spss-stata/#comment-5714)
...
Last weekend I spent about three hours or so learning ```awk``` with this [gem of a book.](http://cm.bell-labs.com/cm/cs/awkbook/)
...
As an aside, I'm also very interested in [```sed```](http://en.wikipedia.org/wiki/Sed).  It's a stream editor, which means that it applies rules to lines of data one at a time and can allow for the processing of HUGE files without actually opening them.

After playing around with awk a bit, I came across [this confusing blog title](http://brenocon.com/blog/2009/09/dont-mawk-awk-the-fastest-and-most-elegant-big-data-munging-language/). The post compares mawk and awk in terms of speed.  **mawk is way faster and can apparently beat C++ and Java-speed!**  The post also talks about potential problems with mawk historically, but it appears that those have been fixed.  Also, I definitely plan to check operations with awk.

See my simple speed comparisons below simply counting 5.2 million records:

{% img center http://i.imgur.com/BUeJ2LM.png %}

mawk is about double the speed of awk!

Finally, the post mentions something exciting.  It imagines an awk on [LLVM](http://en.wikipedia.org/wiki/LLVM). Since the post is a couple years old, I did a little search and found what appears to be [an implementation!](http://lawk.sourceforge.net/)
